(function($){
	...
	$.fn.youTubeEmbed = function(settings){
		...
		$.get(youtubeAPI,{'q':settings.videoID},function(response){
			...
			elements.container = $('<div>').addClass("flashContainer").css({width : settings.width, height : settings.height}).appendTo(elements.originalDIV);
      
      elements.container.addClass("initial");
			...
			elements.control = $('<div>').addClass("controlDiv play ").appendTo(elements.container);

			// If the user wants to show the progress bar:

			if(settings.progressBar){
				elements.progress =	$('<div>').addClass("progressBar").appendTo(elements.container);

				elements.elapsed =	$('<div>').addClass("elapsed").appendTo(elements.progress);
			...
			window['eventListener_'+settings.safeID] = function(status){
              ...
              elements.container.removeClass("initial");
              
              $(".how-notebook .tooltip-custom").fadeOut(150)
              $(".how-notebook .tooltip-popup").fadeOut(150)
